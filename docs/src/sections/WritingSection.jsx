...
export function WritingSection() {
  ...
  return (
    <Sec id="f-writing">
      ...
      <p>정말 꼭 필요할 때만 존칭어를 사용해 문장을 간결하고 직관적으로 유지해요.</p>
      <DoDontTable rows={[
      ...
      <p>단순한 안내를 넘어 사용자의 상황과 감정을 배려하는 사려 깊은 어조를 사용해요.</p>
      <DoDontTable rows={[
      ...
      <p>목적을 해치는 부가 설명은 최소화하고, 하나의 문장에는 하나의 핵심 메시지만 담아요.</p>
      <DoDontTable rows={[
        { do: '정산용 계좌번호를 입력해주세요.', dont: '정산용 계좌번호를 입력해주셔야 정산이 지연되지 않고 정상 입금 처리될 수 있습니다.' },
      ]} />

      <h3>Global Context — 사용자가 속한 시장/지역 이야기 담기</h3>
      <p>온보딩 안내 등에 사용자의 사업 지역이나 시장 이야기를 구체적으로 담아 공감을 형성해요.</p>
      <DoDontTable rows={[
        { do: '지금 아시아 시장의 파트너사들과 정산 자동화를 시작해보세요.', dont: '당사의 뛰어난 글로벌 정산 인프라 솔루션을 지금 즉시 도입해보세요.' },
      ]} />

      <h3>Exclamation Marks — 느낌표는 꼭 필요한 순간에만</h3>
      <p>느낌표를 남발하면 유저가 피로감을 느끼므로 축하나 핵심 완료 순간에만 제한적으로 써요.</p>
      <DoDontTable rows={[
        { do: '가맹점 등록이 완벽하게 처리되었어요! 🎉', dont: '비밀번호를 정확히 입력해주세요!!! / 지금 정산을 완료해보세요!!' },
      ]} />

      <h3>Spaces — 기능명·서비스명·상태값은 붙여쓰기</h3>
      <p>상태값 표기 시에는 단어를 붙여쓰고, 일반 서술문에서는 띄어쓰기 규칙을 준수해요.</p>
      <DoDontTable rows={[
        { do: '정산중', dont: '정산 중 [상태값 표기 시]' },
        { do: '정산 중일 때는 계좌를 변경할 수 없어요. [일반 서술]', dont: '정산중일 때는 계좌를 변경할 수 없어요.' },
      ]} />

      <h3>Periods — 마침표 사용 규칙</h3>
      <p>평서문과 명령문 끝에는 마침표를 사용하며, 아래는 마침표를 생략해요:</p>
